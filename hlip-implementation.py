...
def ik(target_swing, target_com_z, target_stance_x, swing_foot, stance_foot):
    ...
    # print("foot =", foot_xpos_current)
    # print("target =", target)
    # print("error =", error)

    # pseudoinverse
    # Jpinv = np.linalg.pinv(J_leg)

    # damped inverse
    # lam = 0.01
    # Jpinv = J_leg.T @ np.linalg.inv(J_leg @ J_leg.T + lam**2 * np.eye(2))

    # get qdot
    K = 1
    qdot = np.linalg.pinv(J) @ (K*error)
    print("qdot =",qdot)
    return qdot
...
def get_hlip_orbit():
    ...
    # p* and v* are the desired periodic orbit
