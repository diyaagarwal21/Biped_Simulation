...
with mujoco.viewer.launch_passive(m, d) as viewer:
  ...
  while viewer.is_running():
    ...
    hip_y = amplitude * math.sin(2 * np.pi * frequency * step_start)

    cos_theta2 = (hip_y**2 - l1**2 - l2**2) / (2*l1*l2)

    theta2 = np.arccos(np.clip(cos_theta2, -1.0, 1.0))  # Clip to avoid errors due to rounding

    # Calculate theta1 based on theta2 and y_des
    theta1 = np.arctan2(hip_y, 0) - np.arctan2(l2 * np.sin(theta2), l1 + l2 * np.cos(theta2))
    ...
    # Return to origin if q_des is zero
        ...
        d.ctrl[1] = ((Kp * (theta2 - d.qpos[2])) + (Kd * (-d.qvel[2])))
        d.ctrl[0] =  (Kp*(theta1 - d.qpos[1])) + (Kd * (-d.qvel[1]))
